...
for f in PROMPT.md AGENTS.md feature_list.json progress/current.md docs/architecture.md docs/conventions.md docs/verification.md docs/decision-framework.md docs/security.md CHECKPOINTS.md; do
  if [ ! -f "$f" ]; then
...
if [ "$STACK" == "node" ]; then
  ...
  if [ -f "requirements.txt" ]; then
    ...
    warn "pyproject.toml detectado. Asegúrate de que el entorno virtual está activo."
  fi
elif [ "$STACK" == "rust" ]; then
  if [ ! -d "target" ]; then
    warn "target/ no existe. Primera compilación puede ser lenta."
  else
    ok "target/ existe"
  fi
elif [ "$STACK" == "go" ]; then
  if [ ! -d "vendor" ] && go mod download >/dev/null 2>&1; then
    ok "Go modules listos"
  else
    warn "Go modules no verificados. Ejecuta: go mod download"
  fi
fi
