...
## 5. Scripts de orquestación (ya funcionan, no tocar)

Los scripts en `harness/` son genéricos y **no necesitas adaptarlos**. Solo verifica que funcionan:

```bash
./harness/status.py    # Debe mostrar el estado actual
./harness/start.py     # Debe iniciar la primera feature pending
./harness/done.py      # Debe cerrar la sesión (requiere init.sh verde)
```

> Estos scripts protegen al agente de errores comunes: olvidar cambiar `feature_list.json`, cerrar sin tests, o iniciar una sesión cuando ya hay una activa.
...
## 7. Anti-patrones comunes al adaptar
...
- **NO** edites `feature_list.json` a mano. Usa `./harness/start.py` y `./harness/done.py`.
...
./harness/status.py
./harness/start.py
# (el agente trabajaría aquí)
./harness/done.py
```

Todo debe terminar verde.
